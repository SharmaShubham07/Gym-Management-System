...
class Action
{
	...
	function save_user()
	{
		...
		if (empty($id)) {
			$save = $this->db->query("INSERT INTO users values( " . $data . ")");
			return 1;
		} else {
	...
	function save_member()
	{
		...
		if (empty($id)) {
			...
			$save = $this->db->query("INSERT INTO members (member_id,firstname,middlename,lastname,gender,contact,address,email) VALUES($member_id,'$firstname','$middlename','$lastname','$gender',$contact,'$address','$email')	 ");
			if ($save) {
				...
				//$plan = $this->db->query("SELECT * FROM plans where id = $plach_array()['plan']; $data .= ", end_date ='" . date("Y-m-d", strtotime(date('Y-m-d') . ' +' . $plan . ' months')) . "' ";
	...
	{
		extract($_POST);
		//$data = " member_idn_id")->fet = '$member_id' ";
